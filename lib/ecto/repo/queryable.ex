defmodule Ecto.Repo.Queryable do
  ...
  defp preprocess({:&, _, [ix, fields, _]}, value, prefix, context, sources, adapter) do
    case elem(sources, ix) do
      ...
      {source, schema} when is_list(value) ->
        schema = schema_from(schema, value)
  ...
  # If we have a _type field then we are retrieving from
  # an inheritable source and we need to detect the correct
  # schema to load.  Ecto.Schema.Map is generated by the Ecto
  # supervisor when Ecto is started. Since this is a hotspot
  # we take care to minimize performance impact when there is
  # no inheritance - which is the expected norm.  Although we
  # using Enum.at which take O(n) time to access, the _type
  # field is typically at index 0 (although not guaranteed to
  # to be so).  Ecto.Schema.Map is build in Ecto.Supervisor
  # at repo start.  In basic benchmarking on my iMac this
  # take 0.55 µs to execute.
  def schema_from(schema, value) do
    case schema.__schema__(:_type_index) do
      nil ->
        schema
      index ->
        map_name = Enum.at(value, index)
        |> String.to_atom
        apply(Ecto.Schema.Map, :schema_for_source, [map_name])
    end
  end
end
